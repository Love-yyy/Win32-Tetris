...
int* getaddr(int *data, int i, int j, int sidelenght)
{
	return (data + i*sidelenght + j);
}

void change(int* shape, int sidelenght)
{
	int temp;
	for (int count = sidelenght, dt = 0; count>1; count -= 2, dt++)
	{
		for (int i = count - 1; i>0; i--)
		{
			temp = *getaddr(shape, 0 + dt, count - 1 - i + dt, sidelenght);
			*getaddr(shape, dt + 0, dt + count - i - 1, sidelenght) = *getaddr(shape, dt + i, dt + 0, sidelenght);
			*getaddr(shape, dt + i, dt + 0, sidelenght) = *getaddr(shape, dt + count - 1, dt + i, sidelenght);
			*getaddr(shape, dt + count - 1, dt + i, sidelenght) = *getaddr(shape, dt + count - i - 1, dt + count - 1, sidelenght);
			*getaddr(shape, dt + count - i - 1, dt + count - 1, sidelenght) = temp;
		}
	}
}
void InitGame()
...
{
	int count = 0;
	for (int i = CurrentY; i < CurrentY + BlockSet[CurrentBlock].sidelen; i++)
	{
		for (int j = CurrentX; j < CurrentX + BlockSet[CurrentBlock].sidelen; j++)
		{
...
void ToMap(BOOL put=TRUE)
{
	for (int i = CurrentY; i < CurrentY + BlockSet[CurrentBlock].sidelen; i++)
	{
		for (int j = CurrentX; j < CurrentX + BlockSet[CurrentBlock].sidelen; j++)
		{
			if (put)
			{
				if (*getaddr(CurrentBlockBuf, i - CurrentY, j - CurrentX, BlockSet[CurrentBlock].sidelen))
				{
					map[i][j].m_value += *getaddr(CurrentBlockBuf, i - CurrentY, j - CurrentX, BlockSet[CurrentBlock].sidelen);
...
bool MakeBlock()
{
	if (NextBlock == -1)
	{
		CurrentBlock = rand() % 9;
		if (CurrentBlockBuf) delete[]CurrentBlockBuf;
		CurrentBlockBuf = new int[BlockSet[CurrentBlock].sidelen*BlockSet[CurrentBlock].sidelen*sizeof(int)];
		memcpy(CurrentBlockBuf, BlockSet[CurrentBlock].m_pData, BlockSet[CurrentBlock].sidelen*BlockSet[CurrentBlock].sidelen*sizeof(int));
	}
	else
	{
		CurrentBlock = NextBlock;
		if (CurrentBlockBuf) delete[]CurrentBlockBuf;
		CurrentBlockBuf = new int[BlockSet[NextBlock].sidelen*BlockSet[NextBlock].sidelen*sizeof(int)];
		memcpy(CurrentBlockBuf, NextBlockBuf, BlockSet[NextBlock].sidelen*BlockSet[NextBlock].sidelen*sizeof(int));
	}
	//make next block
	NextBlock = rand() % 9;
	if (NextBlockBuf) delete[]NextBlockBuf;
	NextBlockBuf = new int[BlockSet[NextBlock].sidelen*BlockSet[NextBlock].sidelen*sizeof(int)];
	memcpy(NextBlockBuf, BlockSet[NextBlock].m_pData, BlockSet[NextBlock].sidelen*BlockSet[NextBlock].sidelen*sizeof(int));
	//随机旋转n次
	int j1 = rand() % 4 + 1;
	for (int i = 0; i < j1; i++)
		change(NextBlockBuf, BlockSet[NextBlock].sidelen);
	
	CurrentY = 0;
	CurrentX = 7 - BlockSet[CurrentBlock].sidelen / 2;
	ToMap(TRUE);
	bool ret = true;
	int count;
...
void OnKeyDown(char ch)
{
	...
	{
	case 'w':
		temp = new int[BlockSet[CurrentBlock].sidelen*BlockSet[CurrentBlock].sidelen*sizeof(int)];
		memcpy(temp, CurrentBlockBuf, BlockSet[CurrentBlock].sidelen*BlockSet[CurrentBlock].sidelen*sizeof(int));
		ToMap(FALSE);
		change(CurrentBlockBuf, BlockSet[CurrentBlock].sidelen);
		ToMap(TRUE);
		if (GetOverlapsCount())
		{
			ToMap(FALSE);
			memcpy(CurrentBlockBuf, temp, BlockSet[CurrentBlock].sidelen*BlockSet[CurrentBlock].sidelen*sizeof(int));
			ToMap(TRUE);
		}
		delete[]temp;
		break;
...
void DrawGame(HWND hWnd,HDC hdc)
{
	...
	if (NextBlock != -1 && NextBlockBuf)
	{
		...
		int StartX = 20 + 10 * BLOCKSIZE + (3 - BlockSet[NextBlock].sidelen / 2)*BLOCKSIZE;
		int StartY = 10 + (2 - BlockSet[NextBlock].sidelen / 2)*BLOCKSIZE;
		for (int i = 0; i < BlockSet[NextBlock].sidelen; i++)
		{
			for (int j = 0; j < BlockSet[NextBlock].sidelen; j++)
			{
				if (*getaddr(NextBlockBuf, i, j, BlockSet[NextBlock].sidelen))
				{
